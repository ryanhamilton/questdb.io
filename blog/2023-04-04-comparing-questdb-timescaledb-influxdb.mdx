...
- **Data storage (TSM & TSI):** data points are stored in time-structured merge
  ...
  [high cardinality](/glossary/high-cardinality/) (i.e., large number of
  elements in a set), performance can decrease. The proprietary InfluxDB Cloud
  (IOx) claims high cardinality is not an issue anymore.
...
- **Data Retention:** Large deletes are a costly operation in traditional
  [relational databases](/glossary/relational-database/). However, since
  TimescaleDB stores data in chunks, it
