...
I increased the soft limit and change the hard limit to `Integer.MAX` (basically
unlimited).
...
switch to the soft limit, so the existing hard limit should also be set as the
soft limit. Otherwise, suddenly we could start using a much lower value, as
defined by the soft limit (which is 256!).
