...
`ulimit`, `launchctl`, `sysctl`, a mysterious 10240 wall, the frustration, the
sleepless nights...
...
clear up the confusion by running a small test.
...
into QuestDB and queries them after every ten rows.
...
The final table has three columns and is partitioned by day. As the timestamp is
increased by one day in each iteration, each row goes into a new partition. Each
ingested row creates a new directory with three new column files on the disk. It
is easy to see that we need 40 extra file descriptors (10 directories + 30
column files) query by query as we ingest more and more data.

Let's see how far this application can get with different limit settings. The
number of rows ingested successfully will tell us which limit we hit.
...
Max open files limit defaults in the shell:
...
With the above settings, the application could ingest 2510 rows before QuestDB
failed with `errno=24` (max open files limit breach).
...
When I set the hard limit for the session lower than 10240, it was honored.
...
After raising the hard limit above 10240, I hit the JVM's own limit again:

```shell
> ulimit -H -n 12288

2500,"Joe Adams 2510","2026-11-16T12:00:00.000000Z"
```

A bit less than earlier but around the same.

### With the `-XX:-MaxFDLimit` option

Then I added the `-XX:-MaxFDLimit` option in `questdb.sh`. This is how far the
test app got:
...
removing the 10240 wall. Instead, we found a limit of around 4000.

`4 * 950 + 200 = 4000`, closest is the previously set soft limit which is 4096.

Let's increase the soft limit and change the hard limit to Integer.MAX
(basically unlimited).
...
earlier (see above: `kern.maxfilesperproc: 16384`):
...
Only ten rows, but I kind of expected the result. `4 * 10 + 200 = 240`, very
close to 256.
...
I think most of us would expect the hard limit to be used as the absolute limit,
so it would probably make sense to change the soft limit to the hard limit.
...
that (see `ulimit -H -n 8192` above). After removing the 10240 limit, QuestDB
will switch to the soft limit, so the existing hard limit should also be set as
the soft limit. Otherwise, suddenly we could start using a much lower value,
which is currently set as the soft limit (such as 256!).

## Conclusion

The JVM on MacOS has its own built-in max open files limit, set to 10240. This
limit can be removed by passing the -XX:-MaxFDLimit option to the JVM. However,
if the built-in limit is removed, the JVM starts to use the soft limit as the
absolute limit. Soft limits can be set very low, which makes the JVM option
seemingly not working. This should be taken into consideration when using this
JVM option.

## Note about the 10240 limit

You might wonder about the origins of the mysterious 10240 limit in the macOS
JVM. Thanks to my colleague, [Jaromir Hamala](https://github.com/jerrinot), we know that it comes from
